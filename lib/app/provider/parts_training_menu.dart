...
@riverpod
Stream<List<PartsTrainingMenuInfo>> partsTrainingMenuList(Ref ref, AppDataBase database, int partsId) {
  final maxRm = database.trainingDataInfo.rm.max();
  final query = database
      .select(database.partsTrainingInfo)
      .join([leftOuterJoin(database.trainingDataInfo, database.trainingDataInfo.partsTrainingInfo.equalsExp(database.partsTrainingInfo.partsTrainingId), useColumns: false)]);
