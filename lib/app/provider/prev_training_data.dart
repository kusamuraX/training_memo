...
@riverpod
Future<List<TrainingDateData>> prevTrainingData(Ref ref, AppDataBase database, int partsId, int trainingId, DateTime date) async {
  // 前回のトレーニングデータ
  ...
  for (var i = 0; i < 10; i++) {
    ...
    if (lastTrainigInfo.isNotEmpty) {
      ...
      final prevTrainingList = await (database.select(database.trainingDataInfo)
            ..where(
                (tbl) => tbl.bodyPartsInfo.equals(partsId) & tbl.partsTrainingInfo.equals(trainingId) & tbl.trainingDate.isBiggerOrEqualValue(stDate) & tbl.trainingDate.isSmallerOrEqualValue(edDate)))
          .get();
      ...
      trainingDateDataList.add(TrainingDateData(trainingDate: prevDate, totalWeight: double.parse((totalWeight / 1000).toStringAsFixed(2)), trainingList: trainingList));
